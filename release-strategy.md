...
`yeeter` uses a PR-based release flow. A small local script creates the release branch and PR, GitHub Actions tags the merged commit and creates the GitHub Release, and a separate release-published workflow publishes PyPI and deploys docs.
...
5. The published release triggers a separate workflow that publishes PyPI and deploys docs.
...
- `release-publish.yml` handles publishing PyPI and deploying docs after the GitHub Release is published.
...
- wait for the published-release workflow to publish PyPI and deploy docs
