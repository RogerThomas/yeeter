...
`yeeter` is a tiny, typed, signature-driven CLI runner. A function's signature defines the CLI: positional params become positional args, keyword-only params (after `*`) become `--options`. See `README.md` for usage.

## Layout

- `yeeter/` — library source. `_runner.py` builds an `argparse.ArgumentParser` from a function signature; `_metadata.py` defines `Param` for `Annotated` metadata.
- `tests/` — pytest suite.
...
- Keep the public API surface minimal — only `Param`, `YeeterError`, and `run` are exported from `yeeter`.
